...
export class BufferGeometry extends Serializable<BufferGeometrySerialized> {
  ...
  get indices(): BufferAttribute | undefined {return this._indices;}

  set attributes(attributes:{[name:string]: BufferAttribute}) {this._attributes = attributes;}
  set indices(indices:BufferAttribute) {this._indices = indices;}

  getAttribute(name:string) {return this._attributes[name];}
  setAttribute(name:string, attribute:BufferAttribute) {this._attributes[name] = attribute;}

  deleteAttribute(name:string) {
    delete this._attributes[name]; return this;
  }

  removeIndices() {
    this._indices = undefined; return this;
  }
  ...
  calculateNormal(newAttribute=false) {
    ...
    if (!pos) {return;}
    if (newAttribute || !norm) {norm = new BufferAttribute(new Float32Array(pos.length), pos.size);}
