...
export class Vector3 {
  ...
  clampLength(minLength: number, maxLength: number): Vector3 {
    return this.setLength(Math.max(minLength, Math.min(maxLength, this.length())));
  }
  ...
  distanceToVectorSquared(otherVector: Vector3): number {
    const diffX = this.x - otherVector.x, diffY = this.y - otherVector.y, diffZ = this.z - otherVector.z;
  ...
  reflect(planeNormalVector: Vector3): Vector3 {
    const temp = new Vector3();
    temp.copyFrom(planeNormalVector).multiplyScalar( 2 * this.dot(planeNormalVector));
  ...
  angleBetween(otherVector: Vector3) {
    const denominator = Math.sqrt(this.lengthSquared() * otherVector.lengthSquared());
    if (denominator === 0) {
      return Math.PI / 2;
    }
